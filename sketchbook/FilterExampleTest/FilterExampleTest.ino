...
float testFrequency = 4000;                     // test signal frequency (Hz)
float testAmplitude = 128;                   // test signal amplitude
...
void testOnePoleFilters() {
  ...
  FilterOnePole filterOneLowpass( LOWPASS, 8000 );   // create a one pole (RC) lowpass filter
...
//  filterOneHighpassStats.setWindowSecs( windowLength );
  ...
  while( true ) {
    ...
    audioOut =  filterOneLowpass.output() + 128;
    if ( audioOut < 0 ) {
      audioOut = 0;
    }
//    PORTD = filterOneLowpass.output() + 128;
//    Serial.println((int)inputValue + 127);
    PORTD = (int)inputValue + 127;
...
//    Serial.print("audioIn: "); Serial.print(inputValue); Serial.print("   audioOut: "); Serial.println(filterOneHighpass.output());
